...
class Pasien extends AUTH_Controller
{
	...
	public function export()
	{
		...
		$data = $this->M_pasien->select_all_pasien();
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, "ID Klinik");
		...
		foreach ($data as $value) {
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $value->id_klinik);
			...
			if (!$this->upload->do_upload('excel')) {
				...
				foreach ($sheetData as $key => $value) {
					if ($key != 1) {
						...
						$check = $this->M_pasien->check_nama($value['B']);
						...
						if ($check != 1) {
							...
							$resultData[$index]['id_klinik'] = $value['F'];
				...
				if (count($resultData) != 0) {
					$result = $this->M_pasien->insert_batch($resultData);
					if ($result > 0) {
						$this->session->set_flashdata('msg', show_succ_msg('Data Pasien Berhasil diimport ke database'));
						redirect('Pasien');
					}
				} else {
					$this->session->set_flashdata('msg', show_msg('Data Pasien Gagal diimport ke database (Data Sudah terupdate)', 'warning', 'fa-warning'));
					redirect('Pasien');
				}
			}
		}
